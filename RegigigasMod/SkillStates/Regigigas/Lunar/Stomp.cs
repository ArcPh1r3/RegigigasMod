        ...
        private void PerformStomp()
        {
            ...
            if (base.isAuthority)
            {
                ...
                for (int i = 0; i < baseProjectileCount; i++)
                {
                    ...
                    ProjectileManager.instance.FireProjectile(UltChannelState.waveProjectileRightPrefab, footPosition, Util.QuaternionSafeLookRotation(forward), base.gameObject, base.characterBody.damage * Stomp.damageCoefficient, UltChannelState.waveProjectileForce, base.RollCrit(), DamageColorIndex.Default, null, -1f);
                }
            }
        }
